...
time.sleep(120) # for docker compose to run in order and smoothly 

# Scheduler that reads the new hourly data "every hour"
sched = BackgroundScheduler(daemon=True)

# Establish messaging connection, channel and queue
#connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost')) # for localhost 
connection = pika.BlockingConnection(pika.ConnectionParameters(host='rabbitmq')) # for docker 
...
def timed_job():
  ...
  data_file = '{:02d}'.format(yyyy)+"_"+'{:02d}'.format(mm)+"_"+'{:02d}'.format(dd)+"_"+'{:02d}'.format(hh)+"_"+"ActualTotalLoad6.1.A.csv"
  print(data_file)
  ...
  df.drop(df[pd.isna(df['TotalLoadValue'])].index, inplace = True) # drop all NaN entries
  ...
  channel.basic_publish(exchange='', routing_key='ActualTotalLoad', body=year_month, 
...
@app.route('/42_GET_Data_ActualTotalLoad', methods = ['GET'])
def index():
    global json_data

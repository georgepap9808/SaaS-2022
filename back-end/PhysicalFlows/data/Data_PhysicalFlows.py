...
def timed_job():
  ...
  data_file = '{:02d}'.format(yyyy)+"_"+'{:02d}'.format(mm)+"_"+'{:02d}'.format(dd)+"_"+'{:02d}'.format(hh)+"_"+"PhysicalFlows12.1.G.csv"
  print(data_file)
  ...
  df.replace({'FromCountry': countries_dict, 'ToCountry': countries_dict}, inplace = True)
  ...
  channel.basic_publish(exchange='', routing_key='PhysicalFlows', body=year_month, 
...
@app.route('/42_GET_Data_PhysicalFlows', methods = ['GET'])
def index():
    global json_data
